...
def get_last_id():
    logging.info('obteniendo ultima votacion')
    ...
    tbody = soup_html.find(
        id="ContentPlaceHolder1_Content\
...
def key_of_diputade(name: str, dips: List[str]) -> int:
    ...
    if len(result) == 0:
        ...
        # , result[0])
        raise MultiplesDiputadesFound(
            f'Se encontraron múltiples diputades \
                buscando por {name}: {names}', result)
...
def scrap_boletin(vote: Dict[str, str]):
    url = "https://www.camara.cl/legislacion/sala_sesiones/"
    url += f'votacion_detalle.aspx?prmIdVotacion={vote["num_votacion"]}'
    logging.info(f'url detalle votacion: {url}')
    soup_html = BeautifulSoup(urlopen(url), 'html.parser')

    info = soup_html.find(id="info-ficha")\
        .find_all("div", {"class": "datos-ficha"})
    ...
    for i in range(len(totvote)):
        gral[unidecode(totvote[i].text.lower())] = process_data(
            numvote[i].text)
...
def get_votaciones_by_name(name: str, info: bool = False):
    ...
    url = 'https://www.camara.cl/diputados/detalle/'
    url += f'votaciones_sala.aspx?prmId={ dip["pagina"] }#ficha-diputados'
    logging.info(f'url boletin: {url}')
    vote = build_vote_from_url(url)
    vote['diputade'] = name
    logging.debug(f'vote : {vote}')
    return vote
...
def full_scrap(start_id, wanted_results, filepath, verbose=False):
    ...
    for project_n in range(last_project, last_project - n_of_projects, -1):
        try:
            link = f"https://www.camara.cl/legislacion/sala_sesiones/\
                votacion_detalle.aspx?prmIdVotacion={project_n}"
            status, project_results, meta = simple_scrap.scrap_web(
                link, project_n)
